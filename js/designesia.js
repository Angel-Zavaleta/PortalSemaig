...
// one page navigation
		...
		jQuery(".btn").click(function(evn) {
			// Verificar si this.href está definido y es una cadena
			if (typeof this.href === 'string' && this.href.indexOf('#') !== -1) {
				evn.preventDefault();
		
				var target = jQuery(this.hash); // Obtener el destino basado en el hash
				if (target.length) { // Asegurarse de que el elemento existe
					var targetOffset = target.offset().top; // Obtener la posición vertical
		
					// Desplazar suavemente a la posición del destino
					jQuery('html, body').animate({
						scrollTop: targetOffset
					}, 600, 'swing'); // Ajusta la duración y el easing si es necesario
				}
			}
		});
...
// --------------------------------------------------
	...
	jQuery(document).on('scroll', function() {
		...
		jQuery('#homepage nav li a').each(function() {
			...
			if (href && href.indexOf('#') !== -1) { // Verifica si href está definido
				var targetElement = jQuery(href);
				if (targetElement.length) { // Verifica si el elemento objetivo existe
					var targetOffset = targetElement.offset().top;
					if (scrollTop > targetOffset - 140) {
